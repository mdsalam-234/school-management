...
import { Subject } from 'src/app/services/database.objects';
...
export class EditComponent implements OnInit {
  ...
  patchFormValues() {
    if (this.id) {
      this.form.get("subject_name").patchValue(this.datas.subject_name);
      this.form.get("subject_type").patchValue(this.datas.subject_type);
      this.form.get("class").patchValue(this.datas.class);
    }
  }
  ...
  async find(id: number) {
    try {
      ...
      let apiUrl = `${environment.CommonService}subjects/${id}`;
      ...
      if (server instanceof CustomResponse) {
        ...
        if (server.response && server.response.data) {
          this.datas = Subject.fromJSON(server.response.data);
  ...
  async submit() {
    ...
    try {
      ...
      dd.id = this.id
      let apiUrl = `${environment.CommonService}subjects/${this.id}`;
      let server: any = await this._http
