...
import { catchError, delay, map, Observable, Subscriber, takeUntil, throwError } from 'rxjs';
...
export class IndexComponent implements OnInit {
  ...
  reset(): void {
    delete this.filters["filter[roll_number-like]"];
    delete this.filters["filter[name-like]"];
    delete this.filters['filter[gender-like]']
    delete this.filters['filter[class-like]']
    delete this.filters["filter[dob-daterange][0]"];
    delete this.filters["filter[dob-daterange][1]"];
  ...
  async getLists(reset: boolean = false) {
    try {
      ...
      let apiUrl = `${environment.CommonService}students`;
        ...
        if (server instanceof CustomResponse) {
          ...
          if (server.response && server.response.data) {
            ...
            this.students = <Students[]>(
              server.response.data.map((d: any) => Students.fromJSON(d))
            );
